...
class Scorecard:
    ...
    def log_bitrate_choice(self, time: float, quality: int, bitrate: float):
        ...
        Args:
            ...
            bitrate : # of kilobytes the chunk takes up.
        """
    ...
    def count_switches(self, print_output: bool = False) -> int:
        ...
        if print_output:
            print(f'{switches} quality switches detected.\n')
            print(text)
        return switches
    ...
    def output_results(self, verbose: bool = False) -> float:
        ...
        num_switches = self.count_switches(print_output=verbose)

        print('Test results:')
        print(f'\tTotal quality:            {total_quality:.2f}')
        print(f'\tTotal rebuffer time:      {rebuff_time:.2f}')
        print(f'\tTotal number of switches: {num_switches:.2f}')
        print(f'User quality of experience = '
              f'[{self.quality_coeff:.2f}(Quality)'
              f' - {self.rebuffer_coeff:.2f}(Rebuffer Time)'
              f' - {self.switch_coeff:.2f}(Switches)] / (Chunk Count)')

        qoe = total_quality * self.quality_coeff - rebuff_time * self.rebuffer_coeff - num_switches * self.switch_coeff
