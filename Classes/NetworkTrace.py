...
class NetworkTrace:
    ...
    def __init__(self, bandwidths: List[Tuple[float, float]]):
        """
        Args:
            bandwidths : List of tuples, (Start time in seconds, bandwidth in kB/s)
        """
        self.bwlist = bandwidths

    def get_current_timesegment(self, cur_time: float) -> Tuple[float, float]:
        """ Returns the time segement of cur_time as a tuple (Start time in seconds, bandwidth in kB/s) """
    ...
    def simulate_download_from_time(self, time: float, size: float) -> float:
        ...
        Args:
            ...
            size : Size of the download in kB
